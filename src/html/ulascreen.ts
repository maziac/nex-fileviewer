...
export class UlaScreen {
	...
	// The address/offset used to display the ULA screen.
	// This is normally bank 5 but could be changed to bank7 in ZX128.
	// I.e. normally 0x4000, but could be 7*16=0x1C000 for ZX128.
	protected ulaScreenAddressOffset: number;


	/**
	 * Constructor.
	 * @param memory The data buffer.
	 * @param offset The offset inside the 'memory' where the ULA screen begins.
	 */
	constructor(memory: number[], offset: number) {
		this.memory=memory;
		this.ulaScreenAddressOffset = offset;
	}
	...
	public setUlaScreenAddress(screenAddress: number) {
		this.ulaScreenAddressOffset=screenAddress;
	}
	...
	protected createPixels(): Array<number> {
		...
		const screenMem=this.memory;
		const screenBaseAddr=this.ulaScreenAddressOffset;
